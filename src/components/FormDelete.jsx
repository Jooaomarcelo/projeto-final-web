...
export default function FormDelete({ name }) {
    ...
    */
    ...
    return (
        <>
            <button onClick={handleOpenForm} className="text-white text-3xl font-bold bg-red-700 rounded-md p-3">Excluir</button>
            {isOpen &&
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="user-form max-w-lg w-full">
                        <div className="relative">
                            <button type="button" onClick={handleCloseForm} className="absolute top-2 right-2 text-white hover:text-black font-bold">X</button>
                        </div>
                        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                            <h1 className="text-2xl text-center font-bold">Deseja mesmo excluir a república?</h1>
                            <div>
                                <section className="flex gap-2 items-center">
                                    <label htmlFor="password" className="text-nowrap text-lg font-bold">Confirme a senha: </label>
                                    <input
                                        type="password"
                                        id="password"
                                        name="password"
