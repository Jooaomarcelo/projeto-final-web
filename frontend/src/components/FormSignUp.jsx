...
export default function FormLogin(){
    ...
    */
    ...
    return (
        <form onSubmit={handleSubmit} className="user-form form-height">
            <h1 className="font-bold text-3xl text-center">Cadastre sua República</h1>
            <div>
                <input type="text" placeholder="Nome: ex. Poquito Más" name="name" className="input-user-form h-sm:h-10" value={userInputs.name} onChange={handleChange}/>
                {errors && <p className="text-red-500">{errors.name}</p>}
            </div>
            <div>
                <input type="email" placeholder="E-mail" name="email" className="input-user-form h-sm:h-10" value={userInputs.email} onChange={handleChange}/>
                {errors && <p className="text-red-500">{errors.email}</p>}
            </div>
            <div>
                <input type="password" placeholder="Senha" name="password" className="input-user-form h-sm:h-10" value={userInputs.password} onChange={handleChange}/>
                {errors && <p className="text-red-500">{errors.password}</p>}
            </div>
            <div>
                <input type="password" placeholder="Confirme a senha" name="password2" className="input-user-form h-sm:h-10" value={userInputs.password2} onChange={handleChange}/>
                {errors && <p className="text-red-500">{errors.password2}</p>}
            </div>
            <div>
                <input type="number" placeholder="Whatsapp" name="number" className="input-user-form h-sm:h-10" value={userInputs.number ?? ""} onChange={handleChange}/>
